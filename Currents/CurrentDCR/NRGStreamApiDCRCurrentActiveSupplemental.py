...
class NRGStreamApiDCRActiveSupplemental:
...
try:    
    ...
    reedus = settings.futureDRCActiveSupp
    #reedus = int(reedus)
    #f.close()
    nrgStreamApi = NRGStreamApiDCRActiveSupplemental(settings.USERNAME,settings.PASSWORD)         
    # Date range for your data request
    # Date format must be 'mm/dd/yyyy hh:ss'
    fromDateStr = settings.current_Date()
    toDateStr = settings.forward14Days_Date()
