...
try:    
    ...
    #reedus = int(reedus)
    #f.close()
    nrgStreamApi = NRGStreamApiDCR(settings.USERNAME,settings.PASSWORD)         
...
stream_data.rename(columns = {'MW':'DCR'}, inplace = True)
stream_data['Effective Date'] = pd.to_datetime(stream_data['Date/Time'])
stream_data.set_index('Effective Date',drop=True ,inplace=True)
stream_data= stream_data.drop(['Date/Time'], axis=1)
stream_data = stream_data[stream_data.index == tohour]
stream_data['DCR'] = stream_data['DCR'].astype(float)
print(stream_data)
