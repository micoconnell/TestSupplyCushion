...
def currents():
    ...
    import Currents.CurrentDCR.NRGStreamApiDCRCurrent as DCR
    ...
    DCRcurrent.rename(columns = {'MW':'HR_DCR'}, inplace = True)
    DCRcurrent['Effective Date'] = pd.to_datetime(DCRcurrent['Date/Time'])
    DCRcurrent.set_index('Effective Date',drop=True ,inplace=True)
    DCRcurrent= DCRcurrent.drop(['Date/Time'], axis=1)
    DCRcurrent = DCRcurrent[DCRcurrent.index >= tohour]
    DCRcurrent['HR_DCR'] = DCRcurrent['HR_DCR'].astype(float)
    ...
    GEN_DF = pd.DataFrame(genSolar.stream_data)
    GEN_DF.rename(columns = {'MW':'Gen_Solar'}, inplace = True)
    ...
    Current_HourFinalDF['Load'] = AIL_DF['AIL_Demand']
    Current_HourFinalDF['Solar'] = GEN_DF['Gen_Solar']
    Current_HourFinalDF['Wind'] = GEN_DF['Gen_Wind']

    Current_HourFinalDF['DCR'] = DCRcurrent['HR_DCR']
